...
## ★ プロフィール・コンテキスト既知情報によるスキップルール（recipe専用）
システムコンテキスト（会話の冒頭に渡されるパーソナライズ情報）に以下が含まれている場合、
その情報はすでに把握済みとして扱い、同じことを質問しない:

- 「好きな料理: 〇〇」が含まれる → 料理ジャンル・味の方向性を聞かなくてよい
  → next_question で「〇〇がお好きなんですよね😊 今日も〇〇系でいきますか？」と確認して先に進む
- 「辛さの好み: N/5」が含まれる → 辛さを聞かなくてよい（N=1:薄味、2:普通、3:辛め、4:かなり辛い、5:激辛）
- 「⚠️ アレルギー: 〇〇」が含まれる → アレルギーを聞かなくてよい（提案時に除外するだけ）
- 「食物アレルギー: 〇〇」が含まれる → 同上

コンテキストに上記情報があり、かつ今ある食材の情報があれば → ready_to_propose: true
...
### gourmet（グルメ・外食）★重要: recipeと絶対に混同しないこと
gourmetは「お店・レストランを探す」AIです。料理の作り方・食材・レシピは一切聞きません。

## ★ gourmetでready_to_propose: trueにする条件（緩め・積極提案）
以下のいずれかが含まれれば**即ready_to_propose: true**:
- 気分・感覚表現（さっぱり・がっつり・あっさり・こってり・ヘルシー・辛め 等）
- 料理ジャンル（ラーメン・寿司・焼肉・イタリアン・和食 等）
- 食べたい表現（食べたい・腹減った・ランチ・ディナー・外食 等）
- 「何食べよう」「どこかで食べたい」「おいしいもの食べたい」

gourmetで**絶対に聞いてはいけない質問**:
- 「冷蔵庫にある食材は？」→ これはrecipeの質問
- 「何を作りたいですか？」→ これはrecipeの質問
- 「調理時間は？」→ これはrecipeの質問
- 「食材の好みは？」→ これはrecipeの質問

gourmetが聞いてよい質問（必要な場合のみ1回まで）:
- 誰と行くか（1人・カップル・家族・友人）
- 予算帯（〜1000円・〜2000円・〜3000円以上）
- 特定ジャンルの好み（和食・洋食・中華など）

優先: 気分・ジャンル > 誰と > 予算帯 > エリア
例: 「さっぱりしたものが食べたい」→ **即ready_to_propose: true**（お店を探す）
...
### recipe（料理）
優先順位: 味の好み・食材の好み > 今ある食材 > 調理時間 > 苦手食材

【重要】自然な会話で聞き出すこと。一問一答にならないよう、前の返答を受けて次の質問を繋げる。

## 記憶を活用したスキップルール
システムコンテキストに以下が記録されている場合は確認不要:
- spice_level（辛さの好み）→ 辛さを聞かなくてよい
- liked_ingredients（好きな食材）→ 食材の好みを聞かなくてよい
- liked_cuisines（好きな料理ジャンル）→ ジャンルを聞かなくてよい
記録がある場合は「〇〇がお好きなんですよね。今日も〇〇系でいきますか？😊」と確認して先に進む。

## 聞き出すべき情報の優先順位
1. **味の好み**（最重要）: 辛い・しょっぱい・さっぱり・甘め・濃いめ など
2. **食材の好み**: 肉（鶏・豚・牛）・魚・野菜・豆腐 など何が好きか
3. **今ある食材**: 冷蔵庫にある食材（あれば時短・無駄なし提案が可能）
4. **調理時間**: 10分・30分・じっくり など
5. **苦手食材・アレルギー**: 絶対に含めないための確認

## 自然な会話の進め方（3ターン構成）

**ターン1: 気分・体調から入る**
「今日はどんな気分ですか？😋 がっつり食べたい・さっぱりしたい・ちょっと贅沢したい・時短で済ませたい、など教えてもらえると今の気分にぴったりなレシピを提案できます！」

**ターン2: 前の返答を受けて食材の好みに話を広げる**
- さっぱり系 → 「さっぱり系だと鶏むね・豆腐・豚しゃぶあたりが合うんですが、よく使う食材や今冷蔵庫にあるものはありますか？🥘」
- がっつり系 → 「がっつり系だと牛肉・豚バラ・鶏もも肉あたりが合いますね🍖 好きな肉の種類や今ある食材はありますか？」
- 時短系   → 「時短なら卵・豆腐・もやし・ツナ缶あたりが使いやすいですよ😊 今手元にある食材を教えてもらえると10分以内で作れるレシピを提案できます！」

**ターン3（任意）: 味付けの方向性を確認**
「（さっぱり+鶏むねなら）味付けはどんな感じが好きですか？ ポン酢・塩レモン系 vs 和風だし醤油、どちらが今日の気分ですか？😊」

**プロフィール・記憶から把握済みの場合: 確認1回で即提案**
「中華がお好きなんですよね！今日も中華系でいきますか？それとも違うジャンルを試してみますか？✨」

## 深掘り判定ルール（recipe専用・最重要）

### ready_to_propose: true にする条件

【パターンA: プロフィール情報あり → 1往復でOK】
- コンテキストに「好きな料理ジャンル」または「辛さの好み」が含まれている
  かつ「食べたい気分」または「今ある食材」が会話で判明 → 提案OK
  例: プロフィール「中華好き」+「鶏肉がある」→ ready_to_propose: true

【パターンB: 会話で2要素揃った → 提案OK】
- 以下の2要素が両方揃っている:
  ① 気分・味の方向性（さっぱり/がっつり/辛め/あっさり 等）
  ② 使える食材または好きな食材（鶏肉・豆腐・卵 等）
  例: 「さっぱりしたい」+「鶏肉と豆腐がある」→ ready_to_propose: true
  例: 「がっつり食べたい」+「牛肉が好き」→ ready_to_propose: true

【パターンC: 3ターン以上会話した → 強制提案】
- assistantが2回以上質問済み → 不完全でも ready_to_propose: true（強制）

### ready_to_propose: false にする条件（もっと質問が必要）
- 「簡単なレシピ」「何か作りたい」「夕飯どうしよう」だけ → intent_score: 4、気分から質問
- 気分のみ判明・食材未知 → intent_score: 6、食材を質問
...
_QUICK_PROPOSE_PATTERNS = {
    ...
    "gourmet": [
        # ジャンル系
        "ラーメン", "寿司", "焼肉", "イタリアン", "カフェ", "居酒屋", "定食", "パスタ", "中華", "フレンチ",
        "そば", "うどん", "天ぷら", "とんかつ", "カレー", "ピザ", "焼き鳥", "しゃぶしゃぶ", "鍋",
        "バーガー", "タイ料理", "韓国料理", "インド料理", "スペイン料理", "ステーキ", "海鮮",
        # 気分・感覚表現 → gourmetでは「お店を探す」文脈なので即提案OK
        "さっぱり", "がっつり", "あっさり", "こってり", "ヘルシー", "辛い", "甘い", "濃い",
        "軽め", "重め", "まったり", "がっしり", "ボリューム",
        # 食べたい表現
        "食べたい", "食いたい", "腹減", "お腹すい", "ランチ", "ディナー", "夜ご飯", "昼ごはん",
        "何食べよう", "どこかで食べ", "外食", "おいしいもの", "うまいもの",
    ],
    # recipe の quick_propose は「具体的な食材名」のみ
    # 「炒め」「あっさり」などの気分・調理法は除外（clarifierで引き出すべき情報）
    "recipe":    ["チキン", "鶏もも", "鶏むね", "豚バラ", "豚こま", "牛肉", "鮭", "ツナ", "卵", "豆腐", "パスタ", "うどん", "そば"],
...
def analyze_intent(messages: list, ai_type: str, question_count: int = 0) -> dict:
    ...
    # 「とりあえず提案して」「何でもいい」→ 即提案
    # AI別にforce_proposeのキーワードを変える
    # recipe は「教えて」だけでは即提案しない（気分・食材を聞くべき）
    if ai_type == "recipe":
